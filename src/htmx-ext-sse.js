...
;(function () {
    ...
    onEvent: function (name, evt) {
      ...
      switch (name) {
        ...
        case 'htmx:sseBeforeMessage':
          ...
          if (evt.detail.type !== 'PB_CONNECT') return
          const topicNames = parent.getAttribute('sse-swap').split(',')
          const { clientId } = data
          fetch('/api/realtime', {
            method: 'POST',
            headers: {
            ...
            body: JSON.stringify({ clientId, subscriptions: topicNames }),
          })
          evt.preventDefault()
          return
      }
  ...
  function registerSSE(elt) {
    ...
    if (api.getAttributeValue(elt, 'sse-swap')) {
      ...
      var sseEventNames = [
        ...sseSwapAttr.split(',').map((name) => name.trim()),
        'PB_CONNECT',
      ]
  ...
  function ensureEventSourceOnElement(elt, retryCount) {
    ...
    // If element has sse-swap attribute, ensure EventSource exists
    if (api.getAttributeValue(elt, 'sse-swap')) {
      ensureEventSource(elt, '/api/realtime', retryCount)
    }
